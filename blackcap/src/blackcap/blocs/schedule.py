...
def create_schedule(
    ...
    schedule_list: List[Schedule] = [
        scheduler.schedule(schedule_create) for schedule_create in schedule_create_list
    ]
...
def get_schedules(query_params: ScheduleGetQueryParams) -> List[ScheduleDB]:
    ...
    with DBSession() as session:
        try:
            schedule_list: List[ScheduleDB] = session.execute(stmt).scalars().all()
            return schedule_list
...
def update_schedule(schedule_update: ScheduleUpdate) -> ScheduleDB:
    ...
    """
    stmt = select(ScheduleDB).where(ScheduleDB.id == schedule_update.schedule_id)
    with DBSession() as session:
        try:
            schedule_list: List[ScheduleDB] = session.execute(stmt).scalars().all()
            ...
            session.rollback()
            logger.error(
                f"Unable to update schedule: {schedule_update.dict()} due to {e}"
            )
            raise e
...
def delete_schedule(schedule_delete: ScheduleDelete) -> ScheduleDB:
    ...
    """
    stmt = select(ScheduleDB).where(ScheduleDB.id == schedule_delete.schedule_id)
    with DBSession() as session:
        try:
            schedule_list: List[ScheduleDB] = session.execute(stmt).scalars().all()
            ...
            session.rollback()
            logger.error(
                f"Unable to delete schedule: {schedule_delete.dict()} due to {e}"
            )
            raise e
